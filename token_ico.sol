...
contract ICO is Block
{
    ...
    address payable public deposit;//address where ether is deposited

    uint tokenPrice=0.1 ether;
    unit public cap=300 ether;
    ...
    uint public minInvest=0.1ether;

    enum State{berforeStart,afterEnd,running,halted}
    ...
    function changeDepositAdress(address payable newDeposit) public onlymanager
    {
        deposit=newDeposit;
    }
   ...
   function getState() public view returns(State)
   {
    ...
    function invest() payable public return(bool)
    {
        icoState=getState();
        require(icoState=State.running)
        require(msg.value>=minInvest && msg.value<=mazInvest);
        ...
        balances[msg.sender]+=tokens;//inherited from above contract
